    ...
    def add_xp(self, world, n):
        if self.level < self.MAX_LEVEL - 1:
            ...
            if self.xp >= next_level_xp:
                self.level += 1
                world.journal.push_new_line(
                    "Level up! (" + str(self.level + 1) + ")", 9
                )
                self.xp = 0
                self.max_hp += 1
                self.attack += 1
    ...
    def do_pickup(self, map):
        ...
        if item != None:
            if item.type == item.TYPE_WEAPON:
                ...
                map.world.journal.push_new_line("You got the " + self.weapon + "!", 12)
    ...
    def move(self, move_x, move_y, map):
        ...
        if item != None:
            if item.type == item.TYPE_TELEPORTER:
                map_loader.enter_map(
                    map.world, item.to_map_name, self, item.to_tile_x, item.to_tile_y
                )
                return
                ...
                if item.trigger_type == item.TYPE_CASTLE_DOOR:
                    ...
                    map.world.journal.push_new_line("You found the CASTLE KEY!", 7)
                    map.entities.remove(item)
